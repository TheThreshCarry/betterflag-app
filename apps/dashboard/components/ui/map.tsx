...
// MapLibre 6's ESM worker (`new URL(…, import.meta.url)`) breaks Turbopack.
// Stay on v5 + explicit CSP worker from /public.
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    const markReady = () => {
      ...
      // `load` does not re-fire after setStyle(); treat style.load as ready too.
      setIsLoaded(true);
      // Parents with CSS transform/filter/opacity (e.g. stagger-in) can leave the
      // canvas at the wrong size — resize once layout settles.
      requestAnimationFrame(() => {
        map.resize();
      });
    };
    const loadHandler = () => markReady();
    const styleLoadHandler = () => markReady();
...
function MarkerTooltip({
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => {
      ...
      requestAnimationFrame(() => replayPopupEnterAnimation(container));
    };
...
/** Restart entrance animation after MapLibre mounts the popup into the DOM
 * (CSS animations on a detached portal finish before the popup is visible). */
function replayPopupEnterAnimation(root: HTMLElement) {
